 ...
 *
 * Why: vinext 1.0.0-beta.5 supports either `basePath` or `output: 'export'`,
 * but not both at once (with a basePath the build-time prerenderer requests
 * basePath-less URLs and the RSC handler answers 404, so `output: 'export'`
 * fails the build). At runtime the worker handles basePath fine, so instead of
 ...
 *
 * Usage: npm run prerender   (after `npm run build`, from the web/ directory)
 */
...
// top level, basePath-less; copy them into outDir so the Pages artifact is
// self-contained).
const PUBLIC_DIRS = [];
...
try {
  ...
  // `dist/client/project-brush/`, which GitHub Pages maps to
  // https://iizzaya.github.io/project-brush/.
